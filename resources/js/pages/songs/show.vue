    ...
    >
      <div class="bg-cover-container">
        <div
          ...
          :style="{ '--tw-gradient-from': song.image_dominant_color }"
        ></div>
      </div>
      ...
      <div>
        ...
        <div v-if="song.alt_title" class="text-gray-dark dark:text-gray-default">
          Titre alternatif : {{ song.alt_title }}
        </div>
        ...
        <div v-if="song.first_time_played_at" class="text-gray-dark dark:text-gray-default">
          Première diffusion le : {{ moment(song.first_time_played_at).format('L') }}
        </div>
    ...
    <div class="w-full p-4 pb-0 md:p-8">
      <div class="flex flex-col mb-8">
        <div class="flex flex-row px-2 py-2 mb-2 border-b border-gray-lighter dark:border-gray-darker">
          ...
          <div class="flex-auto mx-2 text-xs text-left uppercase truncate text-gray-dark dark:text-gray-default">
            Info
          </div>
          <div
            class="flex-none hidden w-24 mx-2 text-xs text-left uppercase sm:block lg:w-36 text-gray-dark dark:text-gray-default"
          >
            Source
          </div>
          <div class="flex-none w-16 mx-2 text-xs text-left uppercase text-gray-dark dark:text-gray-default">Durée</div>
          <div
            class="flex-none hidden w-20 mx-2 text-xs text-left uppercase sm:block text-gray-dark dark:text-gray-default"
          >
            Taille
          </div>
          <div
        ...
        <div
          ...
          v-bind:key="media.id"
          class="flex flex-row items-center px-2 py-3 mb-2 transition-all duration-200 ease-in-out rounded hover:bg-gray-lightest dark:hover:bg-gray-darker"
        >
          <div
          ...
          <div class="flex-auto mx-2 text-left truncate">
            ...
            <div class="text-xs">
              <span v-if="media.custom_properties.bit_rate"
                >{{ Math.round(media.custom_properties.bit_rate / 1000) }} kbps</span
              >
              <span v-else-if="media.custom_properties.bits_per_raw_sample"
                >{{ media.custom_properties.bits_per_raw_sample }} bits</span
              >
              <span v-else-if="media.custom_properties.bits_per_sample"
                >{{ media.custom_properties.bits_per_sample }} bits</span
              >
          ...
          <div
            class="flex-none hidden w-24 mx-2 text-left truncate sm:block lg:w-36 text-gray-dark dark:text-gray-default"
          >
            <span v-if="media.custom_properties.source == 'youtube'"
              ><i class="mr-1 opacity-50 fa-fw fab fa-youtube"></i> YouTube</span
            >
            <span v-else-if="media.custom_properties.source == 'soundcloud'"
              ><i class="mr-1 opacity-50 fa-fw fab fa-soundcloud"></i> SoundCloud</span
            >
            <span v-else-if="media.custom_properties.source == 'deezer'"
              ><i class="mr-1 opacity-50 fa-fw fab fa-deezer"></i> Deezer</span
            >
            <span v-else-if="media.custom_properties.source == 'qobuz'"
              ><i class="mr-1 opacity-50 fa-fw fas fa-compact-disc"></i> Qobuz</span
            >
          ...
          <div class="flex-none w-10 text-center md:mx-2 text-gray-light dark:text-gray-dark">
            <a
              v-if="song.is_downloadable"
              class="transition-all duration-200 ease-in-out hover:text-gray-dark dark:hover:text-gray-default"
              :href="media.url"
              target="_blank"
            >
            ...
            <button
              v-if="$page.props.user"
              class="ml-2 transition-all duration-200 ease-in-out hover:text-gray-dark dark:hover:text-gray-default"
              @click="destroy_media(media)"
            >
