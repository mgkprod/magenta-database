<template>
    <div class="flex flex-col">
        <div class="flex-row hidden px-2 py-2 mb-2 border-b sm:flex border-gray-darker">
            <div class="flex-none w-8 text-xs text-center uppercase md:mx-2 text-gray-default"></div>
            <div class="flex-none w-10 mx-2 text-xs text-center uppercase text-gray-default"></div>
            <div class="flex-auto mx-2 text-xs text-left uppercase text-gray-default">Titre</div>
            <div class="flex-none hidden w-32 mx-2 text-xs text-left uppercase lg:block text-gray-default">Version</div>
            <div class="flex-none hidden w-32 mx-2 text-xs text-left uppercase sm:block text-gray-default">Disponibilité</div>
            <div class="flex-none hidden w-32 mx-2 text-xs text-left uppercase md:block text-gray-default">Première diff.</div>
        </div>
        <div
            v-for="song in songs"
            v-bind:key="song.id"
            class="flex flex-row items-center py-3 mb-2 transition-all duration-200 ease-in-out rounded cursor-pointer sm:px-2 hover:bg-gray-darker"
            @click="$inertia.get(route('songs.show', song))"
        >
            <div class="flex-none w-8 text-center transition-all duration-200 ease-in-out md:mx-2 text-gray-dark hover:text-gray-default" @click.stop="play_song(song)">
            ...
            <div class="flex-auto mx-2 truncate">
                ...
                <div class="text-sm truncate text-gray-default">
                    {{ song.artist }}
            ...
            <div class="flex-none hidden w-32 mx-2 text-left sm:block text-gray-default">
            ...
            <div class="flex-none hidden w-32 mx-2 text-left md:block text-gray-default">
